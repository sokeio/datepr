...
const version = `/* datepr v${pkg.version},, @license MIT */`;

let DEV_MODE = process.argv.indexOf("--dev") > -1;

const paths = {
  themes: "./src/style/themes/*.styl",
  style: "./src/style/datepr.styl",
  plugins: "./src/plugins",
  l10n: "./src/l10n",
};
...
async function buildScripts() {
  try {
    ...
    const transpiled = await readFileAsync("./dist/datepr.js");
    fs.writeFile("./dist/datepr.min.js", await uglify(transpiled));
  } catch (e) {
    logErr(e);
  }
}
...
async function buildStyle() {
  try {
    ...
    await Promise.all([
      fs.writeFile("./dist/datepr.css", await transpileStyle(src)),
      fs.writeFile("./dist/datepr.min.css", await transpileStyle(src, true)),
...
async function buildThemes() {
  try {
    const themePaths = await glob("./src/style/themes/*.styl");
    await Promise.all(
