  ...
  return (fp: Instance) => {
    ...
    function build() {
      ...
      fp.calendarContainer.classList.add(
        `datepr-monthSelect-theme-${config.theme}`
      );
    }
    ...
    function buildMonths() {
      ...
      if (
        ...
        fp.prevMonthNav.classList.add("datepr-disabled");
      else fp.prevMonthNav.classList.remove("datepr-disabled");

      if (
        fp.config.maxDate &&
        fp.currentYear === fp.config.maxDate.getFullYear()
      )
        fp.nextMonthNav.classList.add("datepr-disabled");
      else fp.nextMonthNav.classList.remove("datepr-disabled");
    }
    ...
    function setCurrentlySelected() {
      ...
      const currentlySelected = fp.rContainer.querySelectorAll(
        ".datepr-monthSelect-month.selected"
      );
      ...
      const month = fp.rContainer.querySelector(
        `.datepr-monthSelect-month:nth-child(${targetMonth + 1})`
      );
      ...
      if (fp.rContainer) {
        ...
        months.forEach((month) => {
          ...
          if (
            ...
            month.classList.add("datepr-disabled");
          } else {
            month.classList.remove("datepr-disabled");
          }
        });
      }
    ...
    function selectMonth(e: Event) {
      ...
      if (eventTarget.classList.contains("datepr-disabled")) return;
