...
function FlatpickrInstance(
  ...
  function init() {
    ...
    /* TODO: investigate this further
      ...
      to scroll up. https://github.com/chmln/datepr/issues/563

      However, most browsers are not Safari and positioning is expensive when used
      in scale. https://github.com/chmln/datepr/issues/1096
    */
  ...
  function build() {
    ...
    if (self.config.inline || self.config.static) {
      ...
      if (self.config.static) {
        const wrapper = createElement("div", "datepr-wrapper");
  ...
  function createDay(
    ...
    if (dateIsEnabled) {
      ...
      dayElement.classList.add("datepr-disabled");
    }
    ...
    if (
      ...
      self.weekNumbers.insertAdjacentHTML(
        "beforeend",
        "<span class='datepr-day'>" + self.config.getWeek(date) + "</span>"
      );
    }
  ...
  function buildMonth() {
    const container = createElement("div", "datepr-month");
  ...
  function buildMonthNav() {
    self.monthNav = createElement<HTMLDivElement>("div", "datepr-months");
    self.yearElements = [];
    self.monthElements = [];
    ...
    self.nextMonthNav = createElement("span", "datepr-next-month");
    ...
    Object.defineProperty(self, "_hidePrevMonthArrow", {
      ...
      set(bool: boolean) {
        if (self.__hidePrevMonthArrow !== bool) {
          toggleClass(self.prevMonthNav, "datepr-disabled", bool);
    ...
    Object.defineProperty(self, "_hideNextMonthArrow", {
      ...
      set(bool: boolean) {
        if (self.__hideNextMonthArrow !== bool) {
          toggleClass(self.nextMonthNav, "datepr-disabled", bool);
  ...
  function buildTime() {
    ...
    self.timeContainer = createElement<HTMLDivElement>("div", "datepr-time");
    self.timeContainer.tabIndex = -1;
    const separator = createElement("span", "datepr-time-separator", ":");

    const hourInput = createNumberInput("datepr-hour", {
    ...
    const minuteInput = createNumberInput("datepr-minute", {
    ...
    if (self.config.enableSeconds) {
      ...
      const secondInput = createNumberInput("datepr-second");
      ...
      self.timeContainer.appendChild(
        createElement("span", "datepr-time-separator", ":")
      );
    ...
    if (!self.config.time_24hr) {
      ...
      self.amPM = createElement(
        ...
        self.l10n.amPM[
          int(
            (self.latestSelectedDateObj
              ? parseInt(self.hourElement.value, 10)
  ...
  function updateWeekdays() {
    ...
    for (let i = self.config.showMonths; i--; ) {
      ...
      <span class='datepr-weekday'>
        ${weekdays.join("</span><span class='datepr-weekday'>")}
      </span>
      `;
    }
  }
  ...
  function buildWeeks() {
    ...
    weekWrapper.appendChild(
      createElement("span", "datepr-weekday", self.l10n.weekAbbreviation)
    );
    const weekNumbers = createElement<HTMLDivElement>("div", "datepr-weeks");
  ...
  function destroy() {
    ...
    if (self.input) {
      ...
      self.input.classList.remove("datepr-input");
  ...
  function isInView(elem: Element) {
    if (self.daysContainer !== undefined)
      return (
        ...
        elem.className.indexOf("datepr-disabled") === -1 &&
  ...
  function onMouseOver(elem?: DayElement, cellClass = "datepr-day") {
    if (
      self.selectedDates.length !== 1 ||
      (elem &&
        (!elem.classList.contains(cellClass) ||
          elem.classList.contains("datepr-disabled")))
    )
      return;
  ...
  function parseConfig() {
    ...
    if (!userConfig.dateFormat && (userConfig.enableTime || timeMode)) {
      const defaultDateFormat =
        datepr.defaultConfig.dateFormat || defaultOptions.dateFormat;
      formats.dateFormat =
    ...
    if (
      ...
      const defaultAltFormat =
        datepr.defaultConfig.altFormat || defaultOptions.altFormat;
      formats.altFormat =
        userConfig.noCalendar || timeMode
  ...
  function setupLocale() {
    if (
      ...
      typeof datepr.l10ns[self.config.locale as LocaleKey] === "undefined"
    )
      self.config.errorHandler(
        new Error(`datepr: invalid locale ${self.config.locale}`)
      );

    self.l10n = {
      ...(datepr.l10ns.default as Locale),
      ...(typeof self.config.locale === "object"
        ? self.config.locale
        : self.config.locale !== "default"
          ? datepr.l10ns[self.config.locale as LocaleKey]
          : undefined),
    };
    ...
    if (
      ...
      datepr.defaultConfig.time_24hr === undefined
    ) {
  ...
  function positionCalendar(customPositionElement?: HTMLElement) {
    ...
    if (!rightMost) {
      ...
      const centerBefore = ".datepr-calendar.centerMost:before";
      const centerAfter = ".datepr-calendar.centerMost:after";
  ...
  function selectDate(e: MouseEvent | KeyboardEvent) {
    ...
    const isSelectable = (day: Element) =>
      day.classList &&
      day.classList.contains("datepr-day") &&
      !day.classList.contains("datepr-disabled") &&
  ...
  function setupInputs() {
    ...
    self.input.classList.add("datepr-input");
  ...
  function setupMobile() {
    ...
    self.mobileInput = createElement<HTMLInputElement>(
      "input",
      self.input.className + " datepr-mobile"
    );
...
function _datepr(
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    try {
      ...
      if (node._datepr !== undefined) {
        node._datepr.destroy();
        node._datepr = undefined;
      }

      node._datepr = FlatpickrInstance(node, config || {});
      instances.push(node._datepr);
    } catch (e) {
      console.error(e);
    }
  }
...
if (
  ...
  // browser env
  HTMLCollection.prototype.datepr = NodeList.prototype.datepr = function (
    config?: Options
  ) {
    return _datepr(this, config);
  };

  HTMLElement.prototype.datepr = function (config?: Options) {
    return _datepr([this], config) as Instance;
  };
}
...
var datepr = function (
  ...
  if (typeof selector === "string") {
    return _datepr(window.document.querySelectorAll(selector), config);
  } else if (selector instanceof Node) {
    return _datepr([selector], config);
  } else {
    return _datepr(selector, config);
  }
} as FlatpickrFn;
...
datepr.localize = (l10n: CustomLocale) => {
  datepr.l10ns.default = {
    ...datepr.l10ns.default,
    ...l10n,
  };
};
datepr.setDefaults = (config: Options) => {
  datepr.defaultConfig = {
    ...datepr.defaultConfig,
...
datepr.parseDate = createDateParser({});
datepr.formatDate = createDateFormatter({});
datepr.compareDates = compareDates;

/* istanbul ignore next */
if (typeof jQuery !== "undefined" && typeof jQuery.fn !== "undefined") {
  (jQuery.fn as any).datepr = function (config: Options) {
    return _datepr(this, config);
  };
}
...
if (typeof window !== "undefined") {
  window.datepr = datepr;
}

export default datepr;
