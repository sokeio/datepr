...
datepr.defaultConfig.closeOnSelect = true;
...
describe("datepr", () => {
  ...
  describe("init", () => {
    ...
    it("shouldn't parse out-of-bounds defaultDate", () => {
      ...
      let enabledDays = fp.days.querySelectorAll(
        ".datepr-day:not(.datepr-disabled)"
      );
  ...
  describe("API", () => {
    ...
    it("onKeyDown closes datepr on Enter when allowInput set to true", () => {
      createInstance({
        enableTime: true,
        allowInput: true,
        altInput: true,
  ...
  describe("UI", () => {
    ...
    describe("mobile calendar", () => {
      ...
      describe("init", () => {
        ...
        it("copy className and adds own", () => {
          ...
          expect(mobileInput.classList).toContain("datepr-mobile");
        });
    ...
    it("valid mouseover behavior in range mode", () => {
      ...
      expect(day(24).classList.contains("datepr-disabled")).toEqual(true);
      ...
      for (let i = 17; i < 22; i++) {
        ...
        expect(day(i).classList.contains("datepr-disabled")).toEqual(false);
      }
    ...
    it("adds disabled class to disabled prev/next month arrows", () => {
      const isArrowDisabled = (which: "prevMonthNav" | "nextMonthNav") =>
        fp[which].classList.contains("datepr-disabled");
      createInstance({
        minDate: "2099-1-1",
        maxDate: "2099-3-4",
        inline: true,
  ...
  describe("Localization", () => {
    ...
    it("By overriding default locale", () => {
      datepr.localize(Russian);
      expect(datepr.l10ns.default.months.longhand[0]).toEqual("Январь");
  ...
  describe("server-side rendering", () => {
    it("can be imported", () => {
      expect(typeof datepr).toEqual("function");
    });
  });
});
